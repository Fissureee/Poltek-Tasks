...
SELECT
    ...
    DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) AS FirstDateOfCurrentMonth,
    EOMONTH(GETDATE()) AS LastDateOfCurrentMonth;
...
SELECT
    ...
    orderdate
FROM Sales.Orders
WHERE DAY(orderdate) >= DATEPART(DAY, EOMONTH(orderdate)) - 4;
...
WHERE YEAR(o.orderdate) = 2007
  AND DATEPART(WEEK, o.orderdate) <= 10;
